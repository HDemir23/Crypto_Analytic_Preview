...
export class CandlestickReversalStrategy implements Strategy {
  public readonly name = "Candlestick Reversal";
  public readonly description =
    "Identifies reversal patterns using candlestick formations like doji, hammer, shooting star, and engulfing patterns";

  // Memoized candlestick pattern recognition (equivalent to useCallback)
  private recognizePatterns = (() => {
    const patternCache = new Map<
      string,
      { result: CandlestickPattern[]; timestamp: number }
    >();
    return (
      priceData: ForecastPoint[],
      ...
      for (let i = 2; i < priceData.length; i++) {
        ...
        if (i >= 1) {
          const prevBodySize = Math.abs(
            previous.avg - priceData[i - 2]?.avg || previous.avg
          );
      ...
      const recentPatterns = patterns.filter(
        (p) => p.day >= priceData.length - lookbackPeriod
      );
  ...
  private determineDojiBias(
    ...
    if (recentTrend === "uptrend") return "bearish";
  ...
  private recognizeStarPattern(
    ...
    if (
      starBodySize < firstBodySize * 0.5 &&
  ...
  private analyzePatternStrength = (() => {
    const strengthCache = new Map<string, { result: any; timestamp: number }>();
    return (
      patterns: CandlestickPattern[],
      priceData: ForecastPoint[],
      volumeIndicator?: IndicatorResult
    ) => {
      const cacheKey = `strength:${patterns.length}:${priceData.length}:${
      ...
      const strongestBullish =
        bullishPatterns.length > 0
          ? bullishPatterns.reduce((max, curr) =>
              curr.confidence > max.confidence ? curr : max
            )
          : null;
      const strongestBearish =
        bearishPatterns.length > 0
          ? bearishPatterns.reduce((max, curr) =>
              curr.confidence > max.confidence ? curr : max
            )
          : null;
      ...
      const result = {
        ...
        strongestBearish,
        dominantSignal:
          bullishConcentration > bearishConcentration
            ? "bullish"
  ...
  private analyzeTrendContext = (() => {
    ...
    return (
      patterns: CandlestickPattern[],
      priceData: ForecastPoint[],
      trendIndicator: IndicatorResult
    ) => {
      ...
      const currentTrend = calculateTrend(trendIndicator.forecast);
      const trendStrength = calculateVolatility(trendIndicator.forecast);
  ...
  async execute(
    ...
    const patternAnalysis = this.analyzePatternStrength(
      patterns,
      priceIndicator.forecast,
      volumeIndicator
    );
    ...
    if (patternAnalysis.dominantSignal === "bullish") {
      ...
      reasons.push(
        `Bullish candlestick pattern: ${strongest.name} (confidence: ${(
      ...
      if (patternAnalysis.bullishConcentration > 0.7) {
        reasons.push(
          `Strong bullish pattern concentration (${(
            patternAnalysis.bullishConcentration * 100
          ).toFixed(0)}%)`
        );
        confidenceScore += 0.2;
      }
      ...
      reasons.push(
        `Bearish candlestick pattern: ${strongest.name} (confidence: ${(
          strongest.confidence * 100
        ).toFixed(0)}%)`
      );
      confidenceScore += strongest.confidence * 0.6;
      ...
      if (patternAnalysis.bearishConcentration > 0.7) {
        reasons.push(
          `Strong bearish pattern concentration (${(
            patternAnalysis.bearishConcentration * 100
          ).toFixed(0)}%)`
        );
        confidenceScore += 0.2;
      }
    ...
    const trendContext = this.analyzeTrendContext(
      patterns,
      priceIndicator.forecast,
      trendIndicator
    );
    ...
    if (recommendation === "buy") {
      ...
      if (trendContext.nearSupport) {
        reasons.push(
          `Price near support level ($${trendContext.supportLevel.toFixed(
            2
          )}) - reversal potential`
        );
      ...
      if (trendContext.nearResistance) {
        reasons.push(
          `Price near resistance level ($${trendContext.resistanceLevel.toFixed(
            2
          )}) - reversal potential`
        );
        confidenceScore += 0.15;
    ...
    if (patternAnalysis.volumeConfirmation > 0.6) {
      reasons.push(
        `Volume supporting reversal pattern (confirmation: ${(
          patternAnalysis.volumeConfirmation * 100
        ).toFixed(0)}%)`
      );
      confidenceScore += 0.1;
    }
    ...
    const forecast = this.generateReversalForecast(
      indicators,
      config,
      recommendation,
      patterns
    ...
    if (uniquePatterns > 2) {
      ...
      reasons.push(
        `Multiple pattern types detected (${uniquePatterns} different patterns)`
      );
    }
    ...
    return (
      ...
      for (let day = 1; day <= config.forecastDays; day++) {
        ...
        const volatilityFactor =
          1 + (Math.random() - 0.5) * 0.1 * avgConfidence;
