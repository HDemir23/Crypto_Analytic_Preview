...
import { fetchHistoricalData } from "../data/fetchPrices";
import { mergeForecasts, calculateMergedForecastStats } from "./mergeForecasts";
import chalk from "chalk";

// Backtest configuration options
export interface BacktestConfig {
  periods: number; // Number of historical periods to test
  forecastDays: number; // Days to forecast in each period
  historicalRange: number; // Historical data range for each forecast
...
export async function runBacktest(
  ...
  debug.log(`Starting backtest for ${symbol} with ${config.periods} periods`);
  console.log(chalk.yellow(`🔍 Running comprehensive backtest analysis...`));
  console.log(chalk.cyan(`   Symbol: ${symbol}`));
  console.log(chalk.cyan(`   Periods: ${config.periods}`));
  console.log(chalk.cyan(`   Forecast Days: ${config.forecastDays}`));
  console.log(
    chalk.cyan(`   Historical Range: ${config.historicalRange} days`)
  );

  const results: BacktestResult[] = [];
  const indicatorAccuracies: { [key: string]: number[] } = {};
  const indicatorErrors: { [key: string]: number[] } = {};

  // Calculate required historical data range
  const totalDaysNeeded =
    config.historicalRange + config.periods * config.forecastDays + 30; // Buffer

  try {
    // Fetch extensive historical data
    debug.log(
      `Fetching ${totalDaysNeeded} days of historical data for backtest`
    );
    const response = await fetchHistoricalData(symbol, totalDaysNeeded);
    ...
    debug.success(`Fetched ${historicalData.length} historical data points`);

    // Run backtest for each period
    for (let period = 0; period < config.periods; period++) {
      console.log(
        chalk.blue(`📊 Testing period ${period + 1}/${config.periods}...`)
      );
      ...
      try {
        ...
        debug.success(
          `Period ${period + 1} completed - Accuracy: ${(
            result.accuracy * 100
          ).toFixed(1)}%`
        );
      } catch (error) {
        debug.error(`Failed to run backtest for period ${period + 1}:`, error);
        console.log(chalk.red(`❌ Period ${period + 1} failed - skipping`));
      }
    }

    // Analyze overall performance
    const analysis = analyzeBacktestResults(
      symbol,
      results,
      config,
      indicatorAccuracies,
    ...
    debug.success(`Backtest completed in ${totalTime}ms`);
...
async function runSingleBacktest(
  ...
  const validationData = historicalData.slice(
    endIndex,
    endIndex + config.forecastDays
  );

  if (
    trainingData.length < config.historicalRange ||
    validationData.length < config.forecastDays
  ) {
    throw new Error(
      `Insufficient data for complete backtest period ${period + 1}`
    );
  }
  ...
  const mergedForecast = mergeForecasts(indicators, config.forecastDays);
  ...
  const indicatorPerformance = indicators.map((indicator) => ({
    ...
    accuracy: calculateAccuracy(
      actualPrices,
      indicator.forecast.map((f) => f.avg)
    ),
    avgError: calculateAverageError(
      actualPrices,
      indicator.forecast.map((f) => f.avg)
    ),
  }));
 ...
 */
function analyzeBacktestResults(
  symbol: string,
  results: BacktestResult[],
  config: BacktestConfig,
  indicatorAccuracies: { [key: string]: number[] },
  indicatorErrors: { [key: string]: number[] }
): BacktestAnalysis {
  if (results.length === 0) {
  ...
  const avgError =
    results.reduce((sum, r) => sum + r.avgError, 0) / results.length;
  ...
  const indicatorPerformance: { [key: string]: any } = {};
  ...
  const sortedIndicators = Object.entries(indicatorPerformance).sort(
...
function calculateAccuracy(actual: number[], predicted: number[]): number {
  ...
  for (let i = 0; i < actual.length; i++) {
    ...
    const range =
      Math.max(actual[i], predicted[i]) - Math.min(actual[i], predicted[i]);
 ...
 */
function calculateAverageError(actual: number[], predicted: number[]): number {
  if (actual.length !== predicted.length || actual.length === 0) {
    return 100;
  }
 ...
 */
function calculateReliability(accuracies: number[]): number {
...
function generateRecommendations(
  ...
  if (overallAccuracy > 0.8) {
    recommendations.push(
      "🎯 Excellent forecast accuracy - system is highly reliable"
    );
  } else if (overallAccuracy > 0.6) {
    recommendations.push(
      "✅ Good forecast accuracy - system shows strong predictive power"
    );
  } else if (overallAccuracy > 0.4) {
    recommendations.push(
      "⚠️  Moderate accuracy - consider adjusting parameters or indicators"
    );
  } else {
    recommendations.push(
      "❌ Low accuracy - review data quality and indicator selection"
    );
  }
  ...
  recommendations.push(
    `🏆 Top performing indicators: ${topIndicators
      .map(([name]) => name)
      .join(", ")}`
  );

  // Reliability assessment
  const reliableIndicators = Object.entries(indicatorPerformance).filter(
    ([, perf]) => (perf as any).reliability > 0.7
  );

  if (reliableIndicators.length > 0) {
    recommendations.push(
      `🔒 Most reliable indicators: ${reliableIndicators
        .map(([name]) => name)
        .join(", ")}`
    );
  }
  ...
  const recentAccuracy =
    recentResults.reduce((sum, r) => sum + r.accuracy, 0) /
  ...
  if (recentAccuracy > overallAccuracy + 0.1) {
    ...
    recommendations.push(
      "📉 Performance declining in recent periods - consider parameter adjustment"
    );
  }
 ...
 */
export function displayBacktestResults(analysis: BacktestAnalysis): void {
  console.log(
    chalk.green(`
┌─────────────────────────────────────────────────────────────────┐
...
🏆 Indicator Performance Rankings:`)
  ...
  const sortedIndicators = Object.entries(analysis.indicatorPerformance).sort(
    ([, a], [, b]) => b.accuracy - a.accuracy
  );
  ...
  sortedIndicators.forEach(([name, perf]) => {
    ...
    const accuracyColor =
      perf.accuracy > 0.7
        ...
        : chalk.red;
    console.log(
      `   ${medal} ${name.padEnd(15)} │ ${accuracyColor(
        (perf.accuracy * 100).toFixed(1) + "%"
      )} │ Error: ${perf.avgError.toFixed(2)}% │ Reliability: ${(
        perf.reliability * 100
      ).toFixed(0)}%`
    );
  });
  ...
  analysis.results.forEach((result) => {
    const accuracyColor =
      result.accuracy > 0.7
        ...
        : chalk.red;
    console.log(
      `   Period ${result.period}: ${accuracyColor(
        (result.accuracy * 100).toFixed(1) + "%"
      )} │ Error: ${result.avgError.toFixed(2)}% │ ${result.startDate} → ${
        result.endDate
      }`
    );
  });
