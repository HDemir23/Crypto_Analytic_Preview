...
export async function calculateSMA(
  ...
  debug.log(
    `Calculating SMA with ${closes.length} data points for ${forecastDays} day forecast`
  );
  ...
  // Determine signals
  const bullish =
    currentPrice > currentShortSMA && currentShortSMA > currentLongSMA;
  const bearish =
    currentPrice < currentShortSMA && currentShortSMA < currentLongSMA;
  ...
  debug.success(`SMA forecast generated: ${forecast.length} points`);
  return forecast;
}
...
function calculateSMAConfidence(
  bullish: boolean,
  bearish: boolean,
  day: number
): number {
