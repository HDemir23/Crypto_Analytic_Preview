...
class BreakoutStrategy {
    constructor() {
        this.name = "Breakout";
        this.description = "Identifies price breakouts from established ranges, support/resistance levels, and volume confirmation";
        ...
        this.calculateSupportResistance = (() => {
            ...
            return (forecast, lookbackPeriod = 20) => {
                ...
                const nearestResistance = resistanceLevels.reduce((prev, curr) => Math.abs(curr.level - currentPrice) <
                    Math.abs(prev.level - currentPrice)
                    ? curr
                    : prev, resistanceLevels[0] || { level: currentPrice * 1.05, strength: 0.5 });
                const nearestSupport = supportLevels.reduce((prev, curr) => Math.abs(curr.level - currentPrice) <
                    Math.abs(prev.level - currentPrice)
                    ? curr
                    : prev, supportLevels[0] || { level: currentPrice * 0.95, strength: 0.5 });
                const result = {
                    resistanceLevels,
                    supportLevels,
                    nearestResistance,
                    ...
                    positionInRange: (currentPrice - nearestSupport.level) /
        ...
        this.analyzeVolumeBreakout = (() => {
            const volumeCache = new Map();
            return (vwapIndicator, lookbackPeriod = 14) => {
        ...
        this.analyzeMomentum = (() => {
            ...
            return (macdIndicator, rsiIndicator) => {
                const cacheKey = `momentum:${macdIndicator.name}:${rsiIndicator?.name || "none"}`;
                ...
                const macdChange = macdValues.length > 1
                ...
                const result = {
                    ...
                    macdChange,
                    rsiMomentum,
                    combinedMomentum: macdTrend === "bullish" ? 1 : macdTrend === "bearish" ? -1 : 0,
            ...
            return (indicators, config, direction) => {
                ...
                for (let day = 1; day <= config.forecastDays; day++) {
                    ...
                    forecast.push({
                        ...
                        confidence: basePoint.confidence * (0.9 - (day / config.forecastDays) * 0.1), // Moderate confidence decay
    ...
    async execute(indicators, config) {
        ...
        if (!vwapIndicator && !macdIndicator && !bollingerIndicator) {
            throw new Error("Breakout strategy requires at least one of: VWAP, MACD, or Bollinger Bands indicators");
        }
        const reasons = [];
        ...
        const baseIndicator = indicators.find((ind) => ind.name.toLowerCase().includes("sma")) ||
            indicators[0];
        const srAnalysis = this.calculateSupportResistance(baseIndicator.forecast, config.lookbackPeriod);
        // Check for resistance breakout (bullish)
        if (srAnalysis.positionInRange > 0.9) {
            reasons.push(`Price near resistance level ($${srAnalysis.nearestResistance.level.toFixed(2)}) - potential breakout`);
            confidenceScore += 0.25;
            recommendation = "buy";
        }
        // Check for support breakout (bearish)
        if (srAnalysis.positionInRange < 0.1) {
            reasons.push(`Price near support level ($${srAnalysis.nearestSupport.level.toFixed(2)}) - potential breakdown`);
            confidenceScore += 0.25;
            recommendation = "sell";
        }
        // Analyze volume breakouts
        if (vwapIndicator) {
            const volumeAnalysis = this.analyzeVolumeBreakout(vwapIndicator, config.lookbackPeriod);
            if (volumeAnalysis.isVolumeBreakout) {
                reasons.push(`Volume breakout detected (${(volumeAnalysis.volumeRatio * 100).toFixed(0)}% above average)`);
                confidenceScore += 0.3;
            ...
            if (volumeAnalysis.volumeSpike) {
                reasons.push(`Significant volume spike detected (${(volumeAnalysis.volumeRatio * 100).toFixed(0)}% above average)`);
                confidenceScore += 0.2;
            }
        }
        // Analyze momentum for breakout confirmation
        if (macdIndicator) {
            const momentumAnalysis = this.analyzeMomentum(macdIndicator, rsiIndicator);
            if (momentumAnalysis.isBullishMomentum) {
                reasons.push(`Bullish momentum confirmed (MACD: ${momentumAnalysis.macdTrend}, change: ${(momentumAnalysis.macdChange * 100).toFixed(2)}%)`);
                confidenceScore += 0.2;
                if (recommendation === "neutral")
                    recommendation = "buy";
            }
            else if (momentumAnalysis.isBearishMomentum) {
                reasons.push(`Bearish momentum confirmed (MACD: ${momentumAnalysis.macdTrend}, change: ${(momentumAnalysis.macdChange * 100).toFixed(2)}%)`);
                confidenceScore += 0.2;
                if (recommendation === "neutral")
                    recommendation = "sell";
            }
            if (momentumAnalysis.momentumStrength > 0.1) {
                reasons.push(`Strong momentum detected (strength: ${momentumAnalysis.momentumStrength.toFixed(2)})`);
                confidenceScore += 0.15;
            }
        }
        // Analyze Bollinger Band breakouts
        if (bollingerIndicator) {
            const bbTrend = (0, types_1.calculateTrend)(bollingerIndicator.forecast);
            const bbVolatility = (0, types_1.calculateVolatility)(bollingerIndicator.forecast);
            if (bbVolatility > 0.03) {
                // High volatility
                reasons.push(`High volatility detected (${(bbVolatility * 100).toFixed(2)}%) - breakout conditions`);
                confidenceScore += 0.15;
                if (bbTrend === "bullish") {
                    if (recommendation === "neutral")
                        recommendation = "buy";
        ...
        if (adxIndicator) {
            const adxTrend = (0, types_1.calculateTrend)(adxIndicator.forecast);
            const adxAvg = (0, types_1.calculateAveragePrice)(adxIndicator.forecast);
            if (adxAvg > 25) {
                // Strong trend
                reasons.push(`Strong trend detected (ADX: ${adxAvg.toFixed(2)}) - breakout potential`);
                confidenceScore += 0.2;
                if (adxTrend === "bullish") {
                    if (recommendation === "neutral")
                        recommendation = "buy";
        ...
        const forecast = this.generateBreakoutForecast(indicators, config, recommendation);
        ...
        if (reasons.length > 2) {
            ...
            reasons.push(`Multiple breakout signals aligned (${reasons.length} indicators)`);
        }
        const signal = {
            recommendation,
            reasons,
...
//# sourceMappingURL=breakoutStrategy.js.map