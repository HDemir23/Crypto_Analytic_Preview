...
exports.MomentumDivergenceStrategy = void 0;
const types_1 = require("./types");
class MomentumDivergenceStrategy {
    constructor() {
        this.name = "Momentum Divergence";
        this.description = "Identifies divergences between price and momentum indicators (RSI, MACD, Stochastic) to predict trend reversals";
        // Memoized calculation for price-momentum divergence (equivalent to useCallback)
        this.analyzeDivergence = (() => {
            const divergenceCache = new Map();
            return (priceIndicator, momentumIndicator, lookbackPeriod = 14) => {
                    ...
                    if (priceLows.length >= 2 && momentumLows.length >= 2) {
                        ...
                        if (priceSlope < -0.01 && momentumSlope > 0.01) {
                            divergences.push({
                                ...
                                priceSlope,
                                momentumSlope,
                                confidence: this.calculateDivergenceConfidence(priceSlope, momentumSlope),
                            });
                        }
                    }
                ...
                const recentDivergences = divergences.filter((d) => d.day >= minLength - lookbackPeriod);
                const strongestDivergence = recentDivergences.length > 0
                    ? recentDivergences.reduce((max, curr) => curr.strength > max.strength ? curr : max)
                    : null;
                const result = {
                    divergences,
                    recentDivergences,
                    strongestDivergence,
                    hasBullishDivergence: recentDivergences.some((d) => d.type === "bullish"),
                    hasBearishDivergence: recentDivergences.some((d) => d.type === "bearish"),
                    avgConfidence: recentDivergences.length > 0
        ...
        this.analyzeRSIDivergence = (() => {
            const rsiCache = new Map();
            return (priceIndicator, rsiIndicator, lookbackPeriod = 14) => {
                ...
                const baseDivergence = this.analyzeDivergence(priceIndicator, rsiIndicator, lookbackPeriod);
                // RSI-specific analysis
                const rsiValues = rsiIndicator.forecast.map((f) => f.avg);
                const currentRSI = rsiValues[rsiValues.length - 1];
                const avgRSI = (0, types_1.calculateAveragePrice)(rsiIndicator.forecast);
                // RSI extreme levels enhance divergence signals
                const isRSIExtreme = currentRSI > 70 || currentRSI < 30;
                const rsiTrend = (0, types_1.calculateTrend)(rsiIndicator.forecast);
                const result = {
                    ...baseDivergence,
                    currentRSI,
                    avgRSI,
                    isRSIExtreme,
                    rsiTrend,
                    enhancedSignal: isRSIExtreme && baseDivergence.recentDivergences.length > 0,
        ...
        this.analyzeMACDDivergence = (() => {
            const macdCache = new Map();
            return (priceIndicator, macdIndicator, lookbackPeriod = 14) => {
                ...
                const baseDivergence = this.analyzeDivergence(priceIndicator, macdIndicator, lookbackPeriod);
                // MACD-specific analysis
                const macdValues = macdIndicator.forecast.map((f) => f.avg);
                const macdTrend = (0, types_1.calculateTrend)(macdIndicator.forecast);
                const macdVolatility = (0, types_1.calculateVolatility)(macdIndicator.forecast);
                // MACD crossover analysis
                const zeroCrossings = this.findZeroCrossings(macdValues);
                const recentCrossings = zeroCrossings.filter((c) => c.index >= macdValues.length - lookbackPeriod);
                const result = {
                    ...baseDivergence,
                    macdTrend,
                    macdVolatility,
                    ...
                    crossoverSupport: recentCrossings.length > 0 &&
        ...
        this.generateDivergenceForecast = (() => {
            ...
            return (indicators, config, direction) => {
                ...
                for (let day = 1; day <= config.forecastDays; day++) {
                    ...
                    forecast.push({
                        ...
                        confidence: basePoint.confidence * (0.85 - (day / config.forecastDays) * 0.25), // Faster confidence decay
    ...
    calculateDivergenceConfidence(priceSlope, momentumSlope) {
    ...
    calculateRSIMomentum(rsiValues) {
    ...
    calculateMACDMomentum(macdValues) {
        if (macdValues.length < 4)
            return "neutral";
    ...
    async execute(indicators, config) {
        ...
        const stochasticIndicator = indicators.find((ind) => ind.name.toLowerCase().includes("stochastic"));
        ...
        if (!rsiIndicator && !macdIndicator && !stochasticIndicator) {
            throw new Error("Momentum Divergence strategy requires at least one momentum indicator (RSI, MACD, or Stochastic)");
        }
        const reasons = [];
        ...
        if (rsiIndicator) {
            const rsiDivergence = this.analyzeRSIDivergence(priceIndicator, rsiIndicator, config.lookbackPeriod);
            if (rsiDivergence.hasBullishDivergence) {
                const strongest = rsiDivergence.strongestDivergence;
                reasons.push(`Bullish RSI divergence detected (strength: ${(strongest.strength * 100).toFixed(2)}%, confidence: ${(strongest.confidence * 100).toFixed(0)}%)`);
                confidenceScore += 0.35;
                recommendation = "buy";
                if (rsiDivergence.enhancedSignal) {
                    reasons.push(`RSI at extreme level (${rsiDivergence.currentRSI.toFixed(1)}) - enhanced divergence signal`);
                    confidenceScore += 0.15;
                }
            }
            else if (rsiDivergence.hasBearishDivergence) {
                const strongest = rsiDivergence.strongestDivergence;
                reasons.push(`Bearish RSI divergence detected (strength: ${(strongest.strength * 100).toFixed(2)}%, confidence: ${(strongest.confidence * 100).toFixed(0)}%)`);
                confidenceScore += 0.35;
                recommendation = "sell";
                if (rsiDivergence.enhancedSignal) {
                    reasons.push(`RSI at extreme level (${rsiDivergence.currentRSI.toFixed(1)}) - enhanced divergence signal`);
            ...
            if (rsiDivergence.rsiMomentum === "strengthening" &&
                recommendation === "buy") {
                reasons.push(`RSI momentum strengthening - supporting bullish signal`);
                confidenceScore += 0.1;
            }
            else if (rsiDivergence.rsiMomentum === "weakening" &&
                recommendation === "sell") {
        ...
        if (macdIndicator) {
            const macdDivergence = this.analyzeMACDDivergence(priceIndicator, macdIndicator, config.lookbackPeriod);
            if (macdDivergence.hasBullishDivergence) {
                const strongest = macdDivergence.strongestDivergence;
                reasons.push(`Bullish MACD divergence detected (strength: ${(strongest.strength * 100).toFixed(2)}%)`);
                confidenceScore += 0.3;
                ...
                const strongest = macdDivergence.strongestDivergence;
                reasons.push(`Bearish MACD divergence detected (strength: ${(strongest.strength * 100).toFixed(2)}%)`);
                confidenceScore += 0.3;
                if (recommendation === "neutral")
                    recommendation = "sell";
                if (macdDivergence.crossoverSupport) {
            ...
            if (macdDivergence.macdMomentum === "accelerating" &&
                recommendation === "buy") {
                reasons.push(`MACD momentum accelerating - supporting bullish signal`);
                confidenceScore += 0.1;
            }
            else if (macdDivergence.macdMomentum === "decelerating" &&
                recommendation === "sell") {
        ...
        if (stochasticIndicator) {
            const stochasticDivergence = this.analyzeDivergence(priceIndicator, stochasticIndicator, config.lookbackPeriod);
            if (stochasticDivergence.hasBullishDivergence) {
                const strongest = stochasticDivergence.strongestDivergence;
                reasons.push(`Bullish Stochastic divergence detected (strength: ${(strongest.strength * 100).toFixed(2)}%)`);
                ...
                const strongest = stochasticDivergence.strongestDivergence;
                reasons.push(`Bearish Stochastic divergence detected (strength: ${(strongest.strength * 100).toFixed(2)}%)`);
                confidenceScore += 0.25;
                if (recommendation === "neutral")
                    recommendation = "sell";
            }
        ...
        const forecast = this.generateDivergenceForecast(indicators, config, recommendation);
        ...
        const divergenceCount = reasons.filter((r) => r.includes("divergence")).length;
        if (divergenceCount > 1) {
            confidenceScore = Math.min(confidenceScore * 1.15, 1.0);
            reasons.push(`Multiple momentum divergences confirmed (${divergenceCount} indicators)`);
        }
        const signal = {
            recommendation,
            reasons,
...
exports.MomentumDivergenceStrategy = MomentumDivergenceStrategy;
//# sourceMappingURL=momentumDivergenceStrategy.js.map