...
class VolatilityBreakoutStrategy {
    constructor() {
        this.name = "Volatility Breakout";
        this.description = "Identifies breakouts based on volatility expansion after periods of compression using Bollinger Bands and ATR";
        // Memoized volatility analysis (equivalent to useCallback)
        this.analyzeVolatilityPattern = (() => {
            const volatilityCache = new Map();
            return (indicator, lookbackPeriod = 20) => {
                ...
                for (let i = lookbackPeriod; i < atr.length; i++) {
                    ...
                    rollingATR.push(window.reduce((sum, val) => sum + val, 0) / window.length);
                }
                const currentVolatility = volatilities[volatilities.length - 1];
                const avgVolatility = volatilities.reduce((sum, val) => sum + val, 0) / volatilities.length;
                const currentATR = rollingATR[rollingATR.length - 1];
                const avgATR = rollingATR.reduce((sum, val) => sum + val, 0) / rollingATR.length;
        ...
        this.analyzeBollingerSqueeze = (() => {
            const squeezeCache = new Map();
            return (bollingerIndicator, lookbackPeriod = 20) => {
                ...
                const avgBandwidth = bandwidths.reduce((sum, val) => sum + val, 0) / bandwidths.length;
                ...
                const result = {
                    ...
                    expansionIntensity,
                    squeezeDuration,
                    breakoutPotential: squeezeDuration > 10
                        ? "high"
                        : squeezeDuration > 5
                            ? "medium"
                            : "low",
        ...
        this.analyzeBreakoutDirection = (() => {
            const directionCache = new Map();
            return (priceIndicator, volumeIndicator, momentumIndicator) => {
                const cacheKey = `direction:${priceIndicator.name}:${volumeIndicator?.name || "none"}:${momentumIndicator?.name || "none"}`;
                const cached = directionCache.get(cacheKey);
                if (cached && Date.now() - cached.timestamp < 60000) {
                    // 1 minute cache
                    return cached.result;
                }
                const priceTrend = (0, types_1.calculateTrend)(priceIndicator.forecast);
                const priceVolatility = (0, types_1.calculateVolatility)(priceIndicator.forecast);
                ...
                if (momentumIndicator) {
                    const momentumTrend = (0, types_1.calculateTrend)(momentumIndicator.forecast);
                    const momentumValues = momentumIndicator.forecast.map((f) => f.avg);
                    const momentumChange = momentumValues.length > 1
                ...
                const combinedSignal = ((priceTrend === "bullish"
                    ? 0.7
                    : priceTrend === "bearish"
                        ? 0.3
                        : 0.5) +
                ...
                const result = {
                    ...
                    predictedDirection: combinedSignal > 0.6
                        ? "bullish"
                        : combinedSignal < 0.4
                            ? "bearish"
                            : "neutral",
        ...
        this.generateVolatilityForecast = (() => {
            const forecastCache = new Map();
            return (indicators, config, direction, volatilityAnalysis) => {
                ...
                for (let day = 1; day <= config.forecastDays; day++) {
                    ...
                    if (volatilityAnalysis.isExpansion) {
                        ...
                        const compressionFactor = 1 - volatilityAnalysis.compressionPhase.strength * 0.02;
                    ...
                    forecast.push({
                        ...
                        confidence: basePoint.confidence * (0.9 - (day / config.forecastDays) * 0.2),
    ...
    detectCompressionPhase(volatilities) {
        ...
        return {
            ...
            strength: compressionLength > 0 ? compressionStrength / compressionLength : 0,
        };
    }
    // Helper method to detect expansion phase
    detectExpansionPhase(volatilities) {
    ...
    async execute(indicators, config) {
        ...
        const momentumIndicator = indicators.find((ind) => ind.name.toLowerCase().includes("macd")) ||
            indicators.find((ind) => ind.name.toLowerCase().includes("rsi"));
        if (!bollingerIndicator) {
            throw new Error("Volatility Breakout strategy requires Bollinger Bands indicator");
        }
        const reasons = [];
        let confidenceScore = 0;
        let recommendation = "neutral";
        // Analyze volatility patterns
        const volatilityAnalysis = this.analyzeVolatilityPattern(priceIndicator, config.lookbackPeriod);
        // Analyze Bollinger Band squeeze
        const squeezeAnalysis = this.analyzeBollingerSqueeze(bollingerIndicator, config.lookbackPeriod);
        // Analyze breakout direction
        const directionAnalysis = this.analyzeBreakoutDirection(priceIndicator, volumeIndicator, momentumIndicator);
        // Volatility compression signals (setup for breakout)
        if (volatilityAnalysis.isCompression && squeezeAnalysis.isSqueeze) {
            reasons.push(`Volatility compression detected - potential breakout setup (compression: ${volatilityAnalysis.compressionPhase.length} periods)`);
            confidenceScore += 0.3;
            if (squeezeAnalysis.breakoutPotential === "high") {
                reasons.push(`Extended volatility squeeze (${squeezeAnalysis.squeezeDuration} periods) - high breakout potential`);
                confidenceScore += 0.2;
            }
        }
        // Volatility expansion signals (breakout in progress)
        if (volatilityAnalysis.isExpansion || squeezeAnalysis.isExpansion) {
            reasons.push(`Volatility expansion detected - breakout in progress (expansion ratio: ${(volatilityAnalysis.volatilityRatio * 100).toFixed(0)}%)`);
            confidenceScore += 0.4;
            // Determine direction based on analysis
            if (directionAnalysis.predictedDirection === "bullish") {
                recommendation = "buy";
                reasons.push(`Bullish breakout direction (confidence: ${(directionAnalysis.directionConfidence * 100).toFixed(0)}%)`);
            }
            else if (directionAnalysis.predictedDirection === "bearish") {
                recommendation = "sell";
                reasons.push(`Bearish breakout direction (confidence: ${(directionAnalysis.directionConfidence * 100).toFixed(0)}%)`);
            }
        }
        // ATR confirmation
        if (volatilityAnalysis.atrRatio > 1.2) {
            reasons.push(`ATR expansion confirms volatility breakout (ratio: ${(volatilityAnalysis.atrRatio * 100).toFixed(0)}%)`);
            confidenceScore += 0.15;
        }
        // Volume confirmation
        if (directionAnalysis.volumeConfirmation > 0.6) {
            reasons.push(`Volume supporting breakout direction (confirmation: ${(directionAnalysis.volumeConfirmation * 100).toFixed(0)}%)`);
            confidenceScore += 0.2;
        }
        // Momentum confirmation
        if (directionAnalysis.momentumConfirmation > 0.6) {
            reasons.push(`Momentum supporting breakout direction (confirmation: ${(directionAnalysis.momentumConfirmation * 100).toFixed(0)}%)`);
            confidenceScore += 0.15;
        }
        // Setup phase (compression without clear direction)
        if (volatilityAnalysis.isCompression && recommendation === "neutral") {
            recommendation = "neutral";
            reasons.push("Volatility compression phase - awaiting directional breakout");
            confidenceScore = Math.max(confidenceScore, 0.4);
        }
        // Generate forecast based on volatility expectations
        const forecast = this.generateVolatilityForecast(indicators, config, recommendation, volatilityAnalysis);
...
//# sourceMappingURL=volatilityBreakoutStrategy.js.map