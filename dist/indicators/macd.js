...
async function calculateMACD(closes, highs, lows, volumes, forecastDays) {
    if (closes.length < 26) {
        throw new Error("MACD requires at least 26 data points");
    }
    index_1.debug.log(`Calculating MACD with ${closes.length} data points for ${forecastDays} day forecast`);
    // MACD parameters
    const fastPeriod = 12;
    const slowPeriod = 26;
    ...
    // Determine signals
    const bullishCrossover = currentMACD > currentSignal && macdTrend > signalTrend;
    const bearishCrossover = currentMACD < currentSignal && macdTrend < signalTrend;
    ...
    for (let day = 1; day <= forecastDays; day++) {
        ...
        const confidence = calculateMACDConfidence(futureHistogram, day, bullishCrossover || bearishCrossover);
        // Validate prediction values to avoid NaN
        if (!isFinite(predictedPrice) ||
            !isFinite(volatility) ||
            !isFinite(confidence)) {
    ...
    index_1.debug.success(`MACD forecast generated: ${forecast.length} points`);
    return forecast;
}
...
function calculateMACDConfidence(histogram, day, crossover) {
    let confidence = 0.75;
