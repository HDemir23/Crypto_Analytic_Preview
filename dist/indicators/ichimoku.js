...
async function calculateIchimoku(closes, highs, lows, volumes, forecastDays) {
    if (closes.length < 52) {
        throw new Error("Ichimoku requires at least 52 data points");
    }
    index_1.debug.log(`Calculating Ichimoku with ${closes.length} data points for ${forecastDays} day forecast`);
    // Ichimoku parameters
    ...
    for (let day = 1; day <= forecastDays; day++) {
        ...
        const confidence = calculateIchimokuConfidence(aboveCloud, belowCloud, inCloud, day, tenkanKijunBullish, cloudBullish);
        forecast.push({
            day,
    ...
    index_1.debug.success(`Ichimoku forecast generated: ${forecast.length} points`);
    return forecast;
}
...
function calculateIchimokuConfidence(aboveCloud, belowCloud, inCloud, day, tenkanKijunBullish, cloudBullish) {
    let confidence = 0.73;
