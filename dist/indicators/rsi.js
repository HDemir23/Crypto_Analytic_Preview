...
async function calculateRSI(closes, highs, lows, volumes, forecastDays) {
    if (closes.length < 14) {
        throw new Error("RSI requires at least 14 data points");
    }
    index_1.debug.log(`Calculating RSI with ${closes.length} data points for ${forecastDays} day forecast`);
    ...
    index_1.debug.success(`RSI forecast generated: ${forecast.length} points`);
    return forecast;
}
// Calculate trend in RSI values
function calculateTrend(rsiValues) {
...
function calculateRSIConfidence(rsi, day) {
    let confidence = 0.8;
