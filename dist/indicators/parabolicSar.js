...
async function calculateParabolicSAR(closes, highs, lows, volumes, forecastDays) {
    if (closes.length < 10) {
        throw new Error("Parabolic SAR requires at least 10 data points");
    }
    index_1.debug.log(`Calculating Parabolic SAR with ${closes.length} data points for ${forecastDays} day forecast`);
    ...
    index_1.debug.success(`Parabolic SAR forecast generated: ${forecast.length} points`);
    return forecast;
}
...
function calculateSARConfidence(distance, day, bullish) {
    let confidence = 0.71;
...
//# sourceMappingURL=parabolicSar.js.map