...
exports.mergeForecasts = mergeForecasts;
exports.calculateMergedForecastStats = calculateMergedForecastStats;
exports.getMergedForecastCacheStats = getMergedForecastCacheStats;
exports.clearMergedForecastCache = clearMergedForecastCache;
exports.debugMergedForecastCacheContents = debugMergedForecastCacheContents;
...
function cleanupMergedForecastCache() {
    const now = Date.now();
    ...
    if (mergedForecastCache.size > MAX_CACHE_SIZE) {
        ...
        const toRemove = entries.slice(0, mergedForecastCache.size - MAX_CACHE_SIZE);
        toRemove.forEach(([key]) => mergedForecastCache.delete(key));
    }
}
// Generate cache key for merged forecasts
function generateMergedForecastCacheKey(indicators, days) {
...
function getCachedMergedForecast(cacheKey) {
    cleanupMergedForecastCache();
    const cached = mergedForecastCache.get(cacheKey);
    if (cached && Date.now() <= cached.expires) {
        index_1.debug.log(`Cache hit for merged forecast: ${cacheKey}`);
        return cached.result;
    }
    return null;
}
// Set cached merged forecast
function setCachedMergedForecast(cacheKey, result) {
    cleanupMergedForecastCache();
 ...
 */
function mergeForecasts(indicators, days) {
    ...
    if (cached) {
        index_1.debug.log(`Using cached merged forecast (${Date.now() - startTime}ms)`);
        return cached;
    }
    index_1.debug.log(`Merging forecasts from ${indicators.length} indicators for ${days} days`);
    // Calculate total weight for normalization
    const totalWeight = indicators.reduce((sum, ind) => sum + ind.weight, 0);
    if (totalWeight === 0) {
        index_1.debug.warn("Total weight is zero, using equal weights");
    ...
    for (let day = 1; day <= days; day++) {
        ...
        if (contributingIndicators === 0) {
            index_1.debug.warn(`No indicators provided forecast for day ${day}`);
            continue;
        }
    ...
    index_1.debug.success(`Merged forecast generated in ${executionTime}ms`);
    // Cache the result
 ...
 */
function calculateMergedForecastStats(mergedForecast, indicators) {
    const totalWeight = indicators.reduce((sum, ind) => sum + ind.weight, 0);
    const avgConfidence = mergedForecast.reduce((sum, point) => sum + point.confidence, 0) /
 ...
 */
function getMergedForecastCacheStats() {
    return {
 ...
 */
function clearMergedForecastCache() {
    mergedForecastCache.clear();
    index_1.debug.log("Merged forecast cache cleared");
}
/**
 * Debug merged forecast cache contents
 */
function debugMergedForecastCacheContents() {
...
//# sourceMappingURL=mergeForecasts.js.map